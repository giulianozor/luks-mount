...
- `-m` / `--mount` — mount point (default: `~/<source-basename>`). A leading `~/` (or a bare `~`) is expanded to your home directory, so `-m '~/data'` never creates a literal `~` directory. If a file already exists at the path, `.mnt` is appended automatically (bounded to 16 candidates). Mounting at the filesystem root (`-m /`) and mounting under a path that is actually a file are refused.
