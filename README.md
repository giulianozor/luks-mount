...
lmount -s <source> -m <mountpoint> -f
```
...
- `-f` / `--force` — force the mount to the destination even when the destination checks would otherwise refuse or divert it: a destination that already exists (including as a file, which would normally trigger the `.mnt` fallback), a non-empty directory, or a directory that is already an active mount point is mounted to as-is. Only valid with `-s`/`--source`. A pre-existing destination is never created, chowned, or recorded as lmount's, and never removed on failure; a genuinely-new destination keeps the normal create/chown/record behavior.
