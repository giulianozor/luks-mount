...
func openAndMount(runCmd func(name string, args ...string) error, runOutput func(name string, args ...string) ([]byte, error), source, keyFile, mountPoint string, force bool) error {
	...
	//
	// When force is set, this whole destination check is skipped: the mount
	// point is used verbatim whether it already exists as a file, is a
	// non-empty directory, or is already an active mount point (the findmnt
	// refusal below). createdMountpoint is still derived so a genuinely-new
	// destination keeps the ownership/cleanup/recording contract, while a
	// pre-existing one is never chown'd, recorded, or removed as lmount's.
	...
	} else if _, err := os.Stat(mountPoint); os.IsNotExist(err) {
		createdMountpoint = true
	}
	...
	// below. Refuse when findmnt reports the target in use, unless force
	// overrides the check; a freshly created mount point cannot already be
	// mounted, so the probe is skipped then too.
	if !createdMountpoint && !force {
	...
	if force && !createdMountpoint {
		// The destination already exists (as a directory, possibly a non-empty
		// one, or even a file). Force mounts to it as-is: nothing is created
		// here, so an existing mount point is neither chown'd, recorded, nor
		// removed as lmount's, and any real failure surfaces from mount itself
		// instead of a mkdir refusal.
		fmt.Printf("Mount point %s already exists; force-mounting over it.\n", mountPoint)
	} else {
