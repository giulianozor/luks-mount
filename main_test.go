...
func TestRunMain(t *testing.T) {
	...
	tests := []struct {
		...
		// -f/--force is only valid with -s/--source (the mount operation), so it
		// must be rejected for every other operation rather than silently dropped.
		{"force without source", []string{"-f"}, 1, "only valid with -s/--source", false, ""},
		{"long force without source", []string{"--force"}, 1, "only valid with -s/--source", false, ""},
		{"force with umount rejected", []string{"-u", "/dev/x", "-f"}, 1, "only valid with -s/--source", false, ""},
		{"long force with create rejected", []string{"-c", "img", "-cs", "32M", "--force"}, 1, "only valid with -s/--source", false, ""},
		{"force with expand rejected", []string{"-x", "/tmp/x.img", "-xs", "1G", "-f"}, 1, "only valid with -s/--source", false, ""},
...
func TestRunMainOperationWiring(t *testing.T) {
	...
	okMount := func(runCmd func(name string, args ...string) error, runOutput func(name string, args ...string) ([]byte, error), source, keyFile, mountPoint string, force bool) error {
		return nil
	}
	...
	boomMount := func(runCmd func(name string, args ...string) error, runOutput func(name string, args ...string) ([]byte, error), source, keyFile, mountPoint string, force bool) error {
		return boom
	}
	...
	t.Run("mount forwards force", func(t *testing.T) {
		var seenForce []bool
		expandOperation, createOperation, umountOperation, mountOperation = okExpand, okCreate, okUmount, okMount
		mountOperation = func(runCmd func(name string, args ...string) error, runOutput func(name string, args ...string) ([]byte, error), source, keyFile, mountPoint string, force bool) error {
			seenForce = append(seenForce, force)
			return nil
		}
		defer func() {
			expandOperation, createOperation, umountOperation, mountOperation = oldExpand, oldCreate, oldUmount, oldMount
		...
		for _, args := range [][]string{
			{"-s", "/dev/__test_dev__"},
			{"-s", "/dev/__test_dev__", "-f"},
			{"--source", "/dev/__test_dev__", "--force"},
		} {
			if code := runMain(args); code != 0 {
				t.Errorf("runMain(%v) = %d, want 0", args, code)
			}
		}
		if len(seenForce) != 3 || seenForce[0] != false || seenForce[1] != true || seenForce[2] != true {
			t.Errorf("mountOperation invoked with force = %v, want [false true true]", seenForce)
		}
	})
