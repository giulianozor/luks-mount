...
func TestOpenAndMount_luks(t *testing.T) {
	t.Run("success default mount", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", "", false)
		if err != nil {
	...
	t.Run("success with mountpoint", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", mp, false)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("an explicit relative mount point is normalized to absolute", func(t *testing.T) {
		...
		err = openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", "rel-mounted-dir", false)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if target == "" {
	...
	t.Run("records the canonical mount point so cleanup matches findmnt", func(t *testing.T) {
		...
		if err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", linkTarget, false); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("mountpoint creation failure is surfaced", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", filepath.Join(locked, "child"), false)
	...
	t.Run("mountpoint path exists as file", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", blocker, false)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("force uses the given path verbatim even when it is a file", func(t *testing.T) {
		// With -f/--force the file-collision fallback is skipped: the mount
		// must target the requested path (the file) as-is rather than diverting
		// to <path>.mnt, and nothing extra may be created.
		var target string
		runCmd := func(name string, args ...string) error {
			if name == "mount" && len(args) >= 2 {
				target = args[len(args)-1]
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", blocker, true)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if target != blocker {
			t.Errorf("force mount target = %q, want the given path %q", target, blocker)
		}
		if _, err := os.Stat(blocker + ".mnt"); !os.IsNotExist(err) {
			t.Error("force must not create a .mnt fallback directory")
		}
	})
	...
	t.Run("mountpoint collides repeatedly with files", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", base, false)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("closes an open mapping when mount point collisions run out", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, src, "", base, false)
		if err == nil || !strings.Contains(err.Error(), "no free mount point") {
			t.Fatalf("expected a bounded-collision error, got %v", err)
		}
		if opens != 1 {
	...
	t.Run("success with keyfile", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", kf, mp, false)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("accepts a trailing-slash key file path", func(t *testing.T) {
		...
		mp := filepath.Join(t.TempDir(), "mnt")
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", kf+string(filepath.Separator), mp, false)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("sniffs a LUKS-magic file without probing cryptsetup", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, src, "", filepath.Join(dir, "mnt"), false)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if cryptCalls != 0 {
	...
	t.Run("rejects a directory key file for a LUKS source before opening", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", dirKey, filepath.Join(t.TempDir(), "mnt"), false)
		if err == nil || !strings.Contains(err.Error(), "is a directory") {
	...
	t.Run("rejects an empty key file for a LUKS source before opening", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", emptyKey, filepath.Join(t.TempDir(), "mnt"), false)
		if err == nil || !strings.Contains(err.Error(), "is empty") {
	...
	t.Run("rejects a missing key file for a LUKS source before opening", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", missing, filepath.Join(t.TempDir(), "mnt"), false)
	...
	t.Run("reports a source that is already open instead of luksOpen", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", filepath.Join(t.TempDir(), "mnt"), false)
	...
	t.Run("rejects an unmappable source name before luksOpen", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__bad name__", "", filepath.Join(t.TempDir(), "mnt"), false)
	...
	t.Run("rejects a key file that is the source itself", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, src, src, filepath.Join(t.TempDir(), "mnt"), false)
	...
	t.Run("rejects a key file hard-linked to the source", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, src, key, filepath.Join(dir, "mnt"), false)
	...
	t.Run("closes an open mapping when refusing the root mount point", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, src, "", "/", false)
		if err == nil || !strings.Contains(err.Error(), "filesystem root") {
	...
	t.Run("empty source is rejected before any probe or open", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "", "", "", false)
	...
	t.Run("force still chowns, creates, and records a fresh destination", func(t *testing.T) {
		recordMountPointsForTest(t)
		var chownArgs []string
		runCmd := func(name string, args ...string) error {
			if name == "chown" {
				chownArgs = append(chownArgs, args...)
			}
			return nil
		}
		runOutput := func(name string, args ...string) ([]byte, error) { return nil, nil }
		...
		var probed bool
		origProbe := mountPointInUse
		mountPointInUse = func(path string) (bool, error) { probed = true; return true, nil }
		defer func() { mountPointInUse = origProbe }()

		// Force only relaxes destination checks; a genuinely-new mount point is
		// still created, chown'd, recorded for cleanup, and (never already
		// mounted) bypasses the probe.
		mp := filepath.Join(t.TempDir(), "fresh")
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", mp, true)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if _, statErr := os.Stat(mp); os.IsNotExist(statErr) {
			t.Error("force must still create a fresh mount point directory")
		}
		if len(chownArgs) < 2 || chownArgs[len(chownArgs)-1] != mp {
			t.Errorf("expected chown on the fresh mount point %q, got %v", mp, chownArgs)
		}
		if probed {
			t.Error("a fresh mount point cannot already be mounted; the probe must be skipped even under force")
		}
		set, setErr := loadMountPoints()
		if setErr != nil {
			t.Fatalf("unexpected error reading state: %v", setErr)
		...
		if _, ok := set[canonMP]; !ok {
			t.Errorf("created mount point was not recorded under force (canonical %q): %v", canonMP, set)
		}
	})

	t.Run("force mounts over an existing non-empty directory as-is", func(t *testing.T) {
		// The destination already exists (here even non-empty); force keeps it
		// untouched (no chown, no record) and mounts to it.
		recordMountPointsForTest(t)
		var chownCalls, mountArgs []string
		runCmd := func(name string, args ...string) error {
			if name == "chown" {
				chownCalls = append(chownCalls, args...)
			}
			if name == "mount" {
		...
		origProbe := mountPointInUse
		mountPointInUse = func(path string) (bool, error) { return true, nil }
		defer func() { mountPointInUse = origProbe }()
		...
		if err := os.WriteFile(filepath.Join(mp, "keep-me"), []byte("data"), 0644); err != nil {
			t.Fatal(err)
		}
		...
		if len(mountArgs) < 2 || mountArgs[1] != mp {
			t.Errorf("expected the mount to target %q, got %v", mp, mountArgs)
		}
		if len(chownCalls) != 0 {
			t.Errorf("force must not chown a pre-existing mount point, got %v", chownCalls)
		}
		if _, statErr := os.Stat(filepath.Join(mp, "keep-me")); os.IsNotExist(statErr) {
			t.Error("force must not remove pre-existing files at the destination")
		}
		set, setErr := loadMountPoints()
		if setErr != nil {
			t.Fatalf("unexpected error reading state: %v", setErr)
		...
		if _, ok := set[canonMP]; ok {
			t.Errorf("a pre-existing mount point must not be recorded as lmount-created: %v", set)
		}
	})
}
...
func TestOpenAndMount_nonLuks(t *testing.T) {
	...
	t.Run("resolves a bare source name to an existing /dev node", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "fd", "", filepath.Join(t.TempDir(), "mnt"), false)
		if err == nil || !strings.Contains(err.Error(), "is a directory") {
	...
	t.Run("rejects a bare source resolving to a character device", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "zero", "", filepath.Join(t.TempDir(), "mnt"), false)
		if err == nil || !strings.Contains(err.Error(), "not a regular file or block device") {
	...
	t.Run("rejects a directory source before mounting", func(t *testing.T) {
		...
		dir := t.TempDir()
		err := openAndMount(runCmd, runOutput, dir, "", "", false)
		if err == nil || !strings.Contains(err.Error(), "is a directory") {
	...
	t.Run("rejects an empty file source before mounting", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, empty, "", "", false)
	...
	t.Run("rejects a nonexistent plain file source before mounting", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, missing, "", "", false)
		if err == nil || !strings.Contains(err.Error(), "does not exist") {
	...
	t.Run("rejects a socket source instead of probing or mounting", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, sock, "", "", false)
		if err == nil || !strings.Contains(err.Error(), "not a regular file") {
			t.Errorf("expected a not-a-regular-file error, got %v", err)
		}
		if mountCalls != 0 {
	...
	t.Run("missing source with -k reports did-not-exist, not not-LUKS", func(t *testing.T) {
		...
		missing := filepath.Join(t.TempDir(), "nope.img")
		err := openAndMount(runCmd, runOutput, missing, "/path/to/key", "", false)
		if err == nil {
	...
	t.Run("missing bare source name is rejected, not probed", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "no-such-device-node", "", filepath.Join(t.TempDir(), "mnt"), false)
		if err == nil {
			t.Fatal("expected an error, got none")
		}
		if !strings.Contains(err.Error(), "does not exist") {
	...
	t.Run("rejects a source nested under a file without probing cryptsetup", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, filepath.Join(file, "sub"), "", "", false)
	...
	t.Run("rejects a symbolic-link loop source without probing cryptsetup", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, loop, "", "", false)
		if err == nil || !strings.Contains(err.Error(), "cannot be accessed") {
	...
	t.Run("rejects -k for a non-LUKS source", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "/path/to/key", filepath.Join(t.TempDir(), "mnt"), false)
...
func TestOpenAndMountBusyMountPoint(t *testing.T) {
	...
	t.Run("refuses a target that is already an active mount", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", busy, false)
	...
	t.Run("mounts when the directory is not a mount point", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", free, false)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("force mounts over a target already in use", func(t *testing.T) {
		// When -f/--force is set, the already-mounted destination check is
		// ignored entirely: the probe must not even run, and the mount must
		// target the busy directory as-is.
		var probed bool
		let(func(path string) (bool, error) { probed = true; return true, nil })
		var mountArgs []string
		runCmd := func(name string, args ...string) error {
			if name == "mount" {
				mountArgs = append(mountArgs, args...)
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", busy, true)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if len(mountArgs) < 2 || mountArgs[1] != busy {
			t.Errorf("expected the mount to target the in-use dir %q, got %v", busy, mountArgs)
		}
		if probed {
			t.Error("force must skip the already-mounted probe entirely")
		}
	})
	...
	t.Run("surfaces a probe failure instead of mounting", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", unknown, false)
	...
	t.Run("skips the probe for a freshly created mount point", func(t *testing.T) {
		...
		err := openAndMount(runCmd, runOutput, "/dev/__test_dev__", "", fresh, false)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if called {
...
func TestOpenAndMountMissingBareName(t *testing.T) {
	...
	err := openAndMount(run, out, "zz-no-such-device", "", "", false)
	if err == nil || !strings.Contains(err.Error(), "does not exist") {
		t.Errorf("expected a does-not-exist error, got %v", err)
	}
}
...
func TestOpenAndMountBareNameResolvedDevIsModeChecked(t *testing.T) {
	...
	err := openAndMount(run, out, bare, "", filepath.Join(t.TempDir(), "mnt"), false)
	if err == nil || !strings.Contains(err.Error(), "is a directory") {
...
func TestOpenAndMountLeadingDashSource(t *testing.T) {
	...
	err := openAndMount(run, out, "-evil.img", "", "", false)
...
func TestOpenAndMountRelativeHomeRefused(t *testing.T) {
	...
	err := openAndMount(func(s string, a ...string) error { return nil }, func(s string, a ...string) ([]byte, error) { return nil, fmt.Errorf("no cryptsetup needed") }, img, "", "", false)
