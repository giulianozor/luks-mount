...
func usageTo(w io.Writer) {
	...
	fmt.Fprintf(w, "  lmount -s <source> [-k <keyfile>] [-m <mountpoint>] [-f]\n\n")
	...
	fmt.Fprintf(w, "  %-29s %s\n", "-f, --force", "Force mount to a destination that already exists or is not empty")
	fmt.Fprintf(w, "  %-29s %s\n", "-h, --help", "Show help")
	fmt.Fprintf(w, "\nNotes:\n")
...
func runMain(args []string) int {
	...
	forceFlag := fs.Bool("f", false, "Force mount to destination even if it already exists or is not empty")
	forceFlagLong := fs.Bool("force", false, "Force mount to destination even if it already exists or is not empty")
	...
	if (*forceFlag || *forceFlagLong) && !mountPresent {
		return failMsg("-f/--force is only valid with -s/--source")
	}
	...
	*mountPoint = trimTrailingSeparators(firstNonEmpty(*mountPoint, *mountPointLong))
	...
	if err := mountOperation(runCmd, runOutput, fullSrc, *keyFile, mp, *forceFlag || *forceFlagLong); err != nil {
		return fail(err)
	}
	return 0
}
