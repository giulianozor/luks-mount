...
func TestExpandHome(t *testing.T) {
	...
	t.Run("an empty HOME makes a ~ path an error, not a cwd-relative path", func(t *testing.T) {
		...
		if _, err := expandHome("~/data"); err == nil {
			t.Error("expandHome must reject a ~ path when HOME is not set")
		}
	})

	t.Run("a relative HOME is rejected for a ~ path", func(t *testing.T) {
		old, had := os.LookupEnv("HOME")
		if err := os.Setenv("HOME", "relative/home"); err != nil {
			t.Fatal(err)
		}
		defer func() {
			if had {
		...
		if _, err := expandHome("~/data"); err == nil {
			t.Error("expandHome must reject a faithful expansion to a relative path")
		}
	})
}
...
func TestCheckMapperName(t *testing.T) {
	tests := []struct {
		...
		{"a\nb", fmt.Errorf("")},
		{"a/b", fmt.Errorf("")},
		{"1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890", fmt.Errorf("")},
		{"1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567", nil},
	}
	for _, tt := range tests {
...
func TestSameFilePath(t *testing.T) {
	...
	tests := []struct {
		...
		{"missing parent falls back to absolute paths", filepath.Join(missingDir, "deep", "img"), filepath.Join(missingDir, "deep", "img"), true},
...
func TestRemoveIfEmpty(t *testing.T) {
	...
	t.Run("never removes the filesystem root under an alternate spelling", func(t *testing.T) {
		// "/." resolves to the root; the guard must match it by file identity
		// rather than an exact "/" string comparison, so the skip note is shown
		// and no removal (or root readdir) happens.
		out := captureStdout(t, func() {
			if err := removeIfEmpty("/."); err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
		})
		if !strings.Contains(out, "Skipping removal of filesystem root") {
			t.Errorf("expected a skip note for the root spelling, got %q", out)
		}
	})
...
func FuzzParseSize(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, in string) {
		...
		if n <= 0 {
			t.Errorf("parseSize(%q) accepted a non-positive value %d", in, n)
		}
	})
}

func FuzzCheckMapperName(f *testing.F) {
	f.Add("vault.img")
	f.Add("-evil")
	f.Add("a b")
	f.Add(".")
	// Seed around the 127-byte device-mapper limit so the boundary stays
	// sharp for the byte-length check.
...
func FuzzParseFindmntTargets(f *testing.F) {
	f.Add([]byte("/mnt/a"))
	f.Add([]byte("/mnt/a\n/mnt/b\n/mnt/a"))
	f.Add([]byte("/mnt/my data"))
	f.Add([]byte("\n"))
	f.Fuzz(func(t *testing.T, in []byte) {
		out := parseFindmntTargets(in)
		for _, m := range out {
			if m == "" {
				t.Errorf("parseFindmntTargets returned an empty target for %q", in)
			}
		}
	})
}

func FuzzSrcName(f *testing.F) {
	f.Add("dir/vault.img")
	f.Add("vault.img")
	f.Add("/dev/sda1")
	f.Fuzz(func(t *testing.T, in string) {
		if n := srcName(in); n == "" && in != "" && !strings.HasSuffix(in, "/") {
			// Only a path whose last element is empty (trailing slash) may
			// yield an empty basename.
			if !strings.HasSuffix(in, "/") {
				t.Errorf("srcName(%q) unexpectedly empty", in)
			}
		}
	})
}

func FuzzTrimTrailingSeparators(f *testing.F) {
	f.Add("/mnt/x/")
	f.Add("/")
	f.Add("a")
	f.Add("a///")
	f.Add("//")
	f.Fuzz(func(t *testing.T, in string) {
		out := trimTrailingSeparators(in)
		if strings.HasSuffix(out, "/") {
			// An all-separator input is a root path and stays untouched.
			if strings.TrimRight(out, "/") != "" {
				t.Errorf("trimTrailingSeparators(%q) = %q still ends in a separator", in, out)
			}
		}
	})
}

func FuzzCalcBlockSize(f *testing.F) {
	f.Add(int64(32 * 1024 * 1024))
	f.Add(int64(512 * 1024 * 1024))
	f.Fuzz(func(t *testing.T, size int64) {
		if size <= 0 {
			return
		}
		bs := calcBlockSize(size)
		if bs <= 0 {
			t.Errorf("calcBlockSize(%d) = %d, must be positive", size, bs)
		}
	})
}

func FuzzResolveKeySize(f *testing.F) {
	f.Add(false, 111, true, 222, 512)
	f.Fuzz(func(t *testing.T, sSet bool, sVal int, lSet bool, lVal, dflt int) {
		got, set := resolveKeySize(sSet, sVal, lSet, lVal, dflt)
		if !sSet && !lSet && (got != dflt || set) {
			t.Errorf("resolveKeySize(none) = (%d, %v), want (%d, false)", got, set, dflt)
		}
		if sSet && (got != sVal || !set) {
			t.Errorf("resolveKeySize(short) = (%d, %v), want (%d, true)", got, set, sVal)
		}
		if !sSet && lSet && (got != lVal || !set) {
			t.Errorf("resolveKeySize(long) = (%d, %v), want (%d, true)", got, set, lVal)
		}
	})
}
