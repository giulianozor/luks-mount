...
func checkMapperName(name string) error {
	...
	// device-mapper names are kernel-side byte arrays of DM_NAME_LEN (128,
	// including the trailing NUL); a name any longer fails cryptsetup's
	// luksOpen with a cryptic "Invalid argument" only after the container was
	// allocated. Names are compared by byte length, matching the kernel.
	if len(name) > 127 {
		return fmt.Errorf("invalid device-mapper name %q: too long (max 127 bytes)", name)
	}
	return nil
}
...
// paths, including relative ones, are returned unchanged. An empty or relative
// $HOME is rejected: expanding "~" against it would silently produce a bare
// relative path (or ""), writing files into the current working directory, so
// the caller gets an error naming the offending flag instead.
func expandHome(p string) (string, error) {
	...
	if home == "" || !filepath.IsAbs(home) {
		return "", fmt.Errorf("expanding %q: home directory %q is not an absolute path", p, home)
	}
	if p == "~" {
		return home, nil
	}
...
func sameFilePath(a, b string) bool {
	canonical := func(p string) string {
		...
		if err != nil {
			// filepath.Abs essentially never fails (no cwd); this branch is a
			// defensive fallback that still yields a deterministic comparison.
...
func removeIfEmpty(path string) error {
	// All three protected paths (root, current working directory, home) are
	// matched by file identity so a spelling variant of the same directory is
	// caught too: a mount target reported as "/.", "//" or a bind-mounted form
	// of the root is still the root. The root check no longer relies on an
	// exact string comparison like the original `path == "/"` guard.
	rootInfo, rootErr := os.Stat(string(filepath.Separator))
